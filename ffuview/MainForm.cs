...
public class MainForm : Form
{
    ...
    public MainForm()
    {
        Text = "ffuview - " + Lang.Get("AnalysisTitle").TrimEnd(' ', ':');
    ...
    private void BuildUi()
    {
        ...
        _aboutItem = new ToolStripMenuItem("关于 ffuview", null, (s, e) => ShowAbout());
    ...
    private void ApplyLanguage()
    {
        string[,] t = {
            ...
            { "关于 ffuview", "關於 ffuview", "About ffuview", "ffuview について", "О программе", "ffuview 정보" },
            { "打开 FFU", "開啟 FFU", "Open FFU", "FFU を開く", "Открыть FFU", "FFU 열기" },
    ...
    {
        if (_ffu == null) return;
        Text = $"ffuview - {Path.GetFileName(path)}";
    ...
    private void ShowAbout()
    {
        MessageBox.Show(this,
            "ffuview v2.0.0.0\n\nFFU 镜像分区与文件浏览器\n支持 V1/V1.1(压缩)/V2 所有 FFU 格式\n只读浏览，不解压不释放\n\n基于 Img2Ffu (Gustave Monce, MIT)\n\nWinStory 2026\nhttps://wiki.win-story.cn\nCompiled by DF4D3110",
            "关于 ffuview", MessageBoxButtons.OK, MessageBoxIcon.Information);
    }
