...
namespace ImageStorageHelper
{
    ...
    public class GptManager : IDisposable
    {
        ...
        /// <summary>Clear all partition entries (reset to empty).</summary>
        public void ClearAllPartitions()
        {
            for (int i = 0; i < _partitions.Count; i++)
                _partitions[i] = new GptPartitionEntry { PartitionType = Guid.Empty, UniqueGuid = Guid.Empty, StartingLba = 0, EndingLba = 0, Attributes = 0, NameBytes = new byte[72] };
        }

        /// <summary>Directly set a partition entry at the given slot index.</summary>
        public void SetPartition(int index, Guid type, ulong startLba, ulong endLba, string name)
        {
            if (index < 0 || index >= _partitions.Count) throw new ArgumentOutOfRangeException(nameof(index));
